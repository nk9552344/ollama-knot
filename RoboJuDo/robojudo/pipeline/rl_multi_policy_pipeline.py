        ...
        for command in commands:
            match command:
                case "[SHUTDOWN]":
                    ...
                    self.env.shutdown()
                case "[ARM]":
                    if getattr(self.env, "enabled", True):
                        logger.warning("[ARM] received but env already armed.")
                    else:
                        logger.warning("[ARM] env enabled — motors now under policy control.")
                        self.env.enabled = True
                case "[DISARM]":
                    if not getattr(self.env, "enabled", True):
                        logger.warning("[DISARM] received but env already disarmed.")
                    else:
                        logger.warning("[DISARM] env disabled — motor commands suppressed.")
