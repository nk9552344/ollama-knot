...
class PolicyInterpManager(PolicyManager):
    ...
    # mimic→loco: 0-tick dwell so loco takes the wheel immediately when [MOTION_DONE]
    # fires — otherwise the frozen last frame of the mimic is held for ~25 ticks and an
    # off-balance ending pose makes the real robot fall before loco can react.
    DURATIONS_MIMIC_LOCO = [0, 75, 0]
    ...
    def __init__(self, cfg: RlLocoMimicPipelineCfg):
        ...
        for entry in self.ctrl_manager.controllers.values():
            if hasattr(entry.inst, "set_mimic_names"):
                entry.inst.set_mimic_names(mimic_names)
        ...
        # Auto-play state
        self._auto_play_idx = 0
        self._auto_play_next_step = self.cfg.auto_play_start_delay_steps
            ...
            match callback:
                case "[MOTION_DONE]":
                    if self.policy_locomotion_mimic_flag == 1:
                        ...
                        if self.cfg.auto_play_mimic:
                            self._auto_play_next_step = self.timestep + self.cfg.auto_play_between_delay_steps

        # Auto-play orchestration: inject [POLICY_MIMIC] once we're in loco + idle.
        if self.cfg.auto_play_mimic and len(self.cfg.mimic_policies) > 0:
            if self._auto_play_idx >= len(self.cfg.mimic_policies) and self.cfg.auto_play_loop:
                self._auto_play_idx = 0
            if self._auto_play_idx < len(self.cfg.mimic_policies):
                is_loco = self.policy_manager.current_policy_id == self.policy_manager.policy_loco_id
                is_idle = self.policy_manager.interp_state == PolicyInterpManager.InterpState.IDLE
                if (
                    is_loco
                    and is_idle
                    and self.policy_locomotion_mimic_flag == 0
                    and self.timestep >= self._auto_play_next_step
                    and "[POLICY_MIMIC]" not in commands
                ):
                    idx = self._auto_play_idx
                    self.policy_manager.policy_mimic_idx = idx
                    self._auto_play_idx += 1
                    logger.info(
                        f"[AutoPlay] Starting mimic {idx + 1}/{len(self.cfg.mimic_policies)}: "
                        f"{self.cfg.mimic_policies[idx].policy_name}"
                    )
            ...
            match command:
                ...
                case cmd if cmd.startswith("[POLICY_SWITCH]"):
                    ...
                    if switch_target == "NEXT":
                        ...
                        # Absolute index. Works in both loco and mimic states
                        # (only updates the selection; the actual swap happens
                        # on the next [POLICY_LOCO]→[POLICY_MIMIC] transition).
                        try:
                            target_idx = int(switch_target)
                        except ValueError:
                            logger.warning(f"Unknown POLICY_SWITCH target: {switch_target!r}")
                        else:
                            if 0 <= target_idx < self.policy_manager.policy_mimic_num:
                                self.policy_manager.policy_mimic_idx = target_idx
                                logger.info(
                                    f"Set mimic policy index to {target_idx}: "
                                    f"{self.cfg.mimic_policies[target_idx].policy_name}"
                                )
                            else:
                                logger.warning(
                                    f"POLICY_SWITCH index {target_idx} out of range "
                                    f"[0, {self.policy_manager.policy_mimic_num})"
                                )
                case "[POLICY_LOCO]":
