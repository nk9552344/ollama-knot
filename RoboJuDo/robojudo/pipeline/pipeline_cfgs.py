...
class RlLocoMimicPipelineCfg(PipelineCfg):
    ...
    """If True, automatically play through mimic_policies in order without user input.
    Each mimic policy must have max_timestep > 0 so the pipeline knows when motion ends."""
    auto_play_start_delay_steps: int = 50
    """Steps to wait (in loco) before the first auto-played mimic starts."""
    auto_play_between_delay_steps: int = 50
    """Steps to wait (in loco) between consecutive auto-played mimics."""
    auto_play_loop: bool = False
    """If True, restart from the first mimic after the last one finishes."""
